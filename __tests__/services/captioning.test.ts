...
describe('CaptioningService', () => {
  ...
  describe('constructor and config', () => {
    it('should create service with default config', () => {
      ...
      // OpenAI temporarily disabled - only Gemini available
      expect(status).toHaveLength(1);
      expect(status.find(s => s.provider === 'gemini')).toBeDefined();
    });
  ...
  /* OpenAI tests temporarily disabled - only Gemini supported
  ...
  */;
  ...
  /* OpenAI provider fallback tests temporarily disabled
  ...
  */
  ...
  describe('confidence evaluation', () => {
    ...
    it('should give high confidence for descriptive captions', async () => {
      (global.fetch as jest.Mock).mockResolvedValue({
        ok: true,
        json: () => Promise.resolve({
          candidates: [{ content: { parts: [{ text: 'A person walking through a sunny park with tall trees' }] } }],
        }),
      });
  ...
  describe('connection tests', () => {
    /* OpenAI connection tests temporarily disabled
  ...
  describe('getProviderStatus', () => {
    it('should return status for Gemini provider', () => {
      const service = new CaptioningService({
        geminiApiKey: 'gemini-key',
      });
    ...
    it('should show Gemini as unavailable without key', () => {
      const service = new CaptioningService();
